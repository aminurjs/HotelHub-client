...
const MyRoom = ({ room, fetchAgain }) => {
  ...
  const { _id, id, title, bookingDays, startDate, endDate, type, size, capacity, price, image } =
    room;
  ...
  const handleCancelBooking = (type) => {
    ...
    swal({
      ...
      if (willDelete) {
        axios
          .delete(`/booking/delete?_id=${_id}&id=${id}&type=${type}&currentDate=${currentDate}`)
          .then((response) => {
            if (response.data.result.modifiedCount > 0 && response.data.result2.modifiedCount > 0) {
      ...
      <div className="flex items-center gap-6 mb-5 pb-10 border-b border-gray-200">
        ...
        <div className="md:w-1/4 space-y-2">
          ...
          {room.status === "processing" && (
            <div>
              <button
                onClick={() => handleCancelBooking("checkout")}
                data-tip="only cancel possible before check-in date."
